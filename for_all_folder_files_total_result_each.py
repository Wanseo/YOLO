...
# bring data (input as a txt file)
folders = open('/Users/jowanseo/Desktop/{}.txt'.format(TEXT_FILE),'r').read().splitlines()
for one_folder in folders:
    ...
    count = 0
    cwd = '/'
    dirname = 'Users/jowanseo/Desktop/{}'.format(one_folder)
    files = os.listdir('/Users/jowanseo/Desktop/{}'.format(one_folder))
    ...
    f = open('/Users/jowanseo/Desktop/xml_results/{}.csv'.format(one_folder), 'w', newline='')
    wr = csv.writer(f)
    wr.writerow(['file name {}'.format(one_folder)])
    wr.writerow(['# of image {}'.format(count)])
    wr.writerow(['step size {}'.format(PIXEL)])
    f.close()

    with open('/Users/jowanseo/Desktop/xml_results/{}.csv'.format(one_folder), 'a') as f:
